...
class Evaluator(object):
    ...
    def aggregate(self, predictor, store_image=0):
        ...
        while frame <= self.start + 70:
            ...
            env.render()
            if frame != self.start + 20:
                obss.append(obs)
            frame = frame + 5
        # env.close()
        frames = np.linspace(self.start + 25, self.start + 70, 10)
...
def eval(input_file, predictor):
    ...
    evaluator = Evaluator(scenes, file_name=file_name, sample_rate=sample_rate, start = reader.start)  # nonlinear_scene_index
